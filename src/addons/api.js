...
import enMessages from './addons-l10n/en.json';
...
// Working set of messages for the current locale. Starts empty and is populated by
// loadTranslations() so we can reset it when the language changes at runtime.
let addonMessages = {};
// Bumped whenever translations are reloaded so cached IntlMessageFormat objects are invalidated.
let translationGeneration = 0;
let loadedLocale = getLocale();

const loadTranslations = async (locale) => {
    const resolved = Object.prototype.hasOwnProperty.call(l10nEntries, locale)
        ? locale
        : locale.split('-')[0];
    // Reset to the English baseline before overlaying the current locale's strings, so switching
    // languages does not leave stale entries from a previous locale behind.
    Object.keys(addonMessages).forEach((key) => delete addonMessages[key]);
    Object.assign(addonMessages, enMessages);
    if (Object.prototype.hasOwnProperty.call(l10nEntries, resolved)) {
        const localeMessages = await l10nEntries[resolved]();
        Object.assign(addonMessages, localeMessages);
    }
    translationGeneration++;
};
const addonMessagesPromise = loadTranslations(loadedLocale);

// Reload addon translations whenever the editor language changes at runtime, so addon-provided
// UI (e.g. context menu items like "create folder") follows the current language.
reduxInstance.addEventListener('statechanged', () => {
...
class AddonRunner {
    ...
    _msg (key, vars, handler) {
        ...
        const cacheKey = `${namespacedKey}@${locale}#${translationGeneration}`;
        if (this.messageCache[cacheKey]) {
            return this.messageCache[cacheKey].format(vars);
        }
        ...
        const messageFormat = new IntlMessageFormat(translation, locale);
        this.messageCache[cacheKey] = messageFormat;
