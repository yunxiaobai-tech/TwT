...
import langImage from './icons/lang.svg';
...
// 语言切换上下文：'zh' 显示中文翻译（无则回退英文原文），'en' 直接显示英文原文
...
document.title = `TwT - ${settingsTranslations.title}`;
...
// 这些插件已从插件列表隐藏（功能改到编辑器顶栏"个性化"里控制）
const HIDDEN_ADDONS = ['editor-stage-left'];

const filterAddonsBySupport = () => {
    const supported = {};
    const unsupported = {};
    for (const [id, manifest] of Object.entries(importedAddons)) {
        if (HIDDEN_ADDONS.includes(id)) {
            continue;
        }
...
const AddonName = ({id, manifest}) => {
    const lang = React.useContext(LangContext);
    return (lang === 'en' ? manifest.name : (addonTranslations[`${id}/@name`] || manifest.name));
};
AddonName.propTypes = {
    id: PropTypes.string,
    manifest: PropTypes.shape({
...
const AddonDescription = ({id, manifest}) => {
    const lang = React.useContext(LangContext);
    return (lang === 'en' ? manifest.description : (addonTranslations[`${id}/@description`] || manifest.description));
};
AddonDescription.propTypes = {
    id: PropTypes.string,
    manifest: PropTypes.shape({
        description: PropTypes.string
    })
        ...
        <div className={styles.addonHeader}>
            <label className={styles.addonTitle}>
                ...
                <div className={styles.addonTitleText}>
                    <AddonName id={id} manifest={manifest} />
                </div>
                {extended && (
            ...
            {!settings.enabled && (
                <div className={styles.inlineDescription}>
                    <AddonDescription id={id} manifest={manifest} />
                </div>
            )}
                ...
                <div className={styles.header}>
                    <div className={styles.section}>
                        ...
                        <button
                            ...
                            onClick={() => this.setState({lang: this.state.lang === 'zh' ? 'en' : 'zh'})}
                            title={this.state.lang === 'zh' ? '切换到英文' : '切换到中文'}
                        >
                            <img
                                ...
                                draggable={false}
                            />
                            <span>{this.state.lang === 'zh' ? 'EN' : '中'}</span>
                        </button>
                    </div>
                    {this.state.dirty && (
                        <Dirty
