...
(function (Scratch) {
  'use strict';
  // 注意：在 TwT/unsandboxed 模式下，Scratch 运行时会在注入脚本前把 window.Scratch
  // 设为带 extensions.register 的对象。若此处拿不到，多半是扩展被归到了 iframe 沙箱
  // （例如扩展 URL 不在信任列表内）。不再静默 return，改为明确报错，便于排查。
  if (!(Scratch && Scratch.extensions && typeof Scratch.extensions.register === 'function')) {
    throw new Error('[音频编码器] 扩展注册环境不可用：当前可能被加载到了不兼容的沙箱环境（iframe）。' +
      '请确认扩展 URL 属于本站同源或已加入安全管理的信任列表。');
  }
